...
class Server {
    ...
    onHttpServerOpen() {
        ...
        // Done
        Logger.info("Empezando el juego en servidor, en puerto " + this.config.serverPort);
        Logger.info("El modo de Juego es " + this.mode.name);
        // Player bots (Experimental)
        if (this.config.serverBots) {
            for (var i = 0; i < this.config.serverBots; i++)
                this.bots.addBot();
            Logger.info("se a añadido " + this.config.serverBots + " bots jugadores");
        }
    ...
    mainLoop() {
        ...
        if (this.ticks > this.config.serverRestart) {
            ...
            if (this.config.serverBots) {
                for (var i = 0; i < this.config.serverBots; i++)
                    this.bots.addBot();
                Logger.info("Se añadio " + this.config.serverBots + " Jugadores Bots");
            }
            ;
            this.commands;
            this.ticks = 0;
    ...
    loadFiles() {
        ...
        try {
            if (!fs.existsSync(fileNameBadWords)) {
                ...
                this.badWords = words;
                Logger.info(this.badWords.length + " malas palabras cargadas");
            }
        }
        catch (err) {
        ...
        try {
            ...
            this.userList = list;
            Logger.info(this.userList.length + " registros de usuario cargados.");
        }
        catch (err) {
            Logger.error(err.stack);
        ...
        try {
            if (fs.existsSync(fileNameIpBan)) {
                ...
                Logger.info(this.ipBanList.length + " Registros de prohibición de IP cargados.");
            }
            else {
    ...
    startStatsServer(port) {
        ...
        this.httpServer.listen(port, function () {
            // Stats server
            Logger.info("Servidor de estadísticas iniciado en el puerto " + port);
